...
##### Looking at indicator time frequencies
I first want to look at the period of time between instances for each indicator. This will give some insight into what kind of time series data is contained within the whole dataset and provide a table with the time period for each indicator which can be used later on. When deciding on which question we want to answer with the dataset it may be preferable to use multiple indicators of the same frequency so that we don't lose any information when merging the indicator subsets.
...
# make new column with approximate length (days and months)
...
indicator.info
```
We may want to answer a question regarding the impact COVID-19 has had on travel so we will look at the indicators `Daily border crossings - arrivals` and `Daily border crossings - departures`.
```{r}
arrivals <- covid.1 %>% 
  filter(indicator_name == "Daily border crossings - arrivals" & series_name == "Total") %>% 
  select(parameter, value) %>% 
  rename(arrivals = value)

departures <- covid.1 %>% 
  filter(indicator_name == "Daily border crossings - departures" & series_name == "Total") %>% 
  select(parameter, value) %>% 
  rename(departures = value)

flights <- left_join(arrivals, departures, by = "parameter")
flights$arrivals <- as.numeric(flights$arrivals)
flights$departures <- as.numeric(flights$departures)

ggplot(flights, aes(x = parameter), size = 0.1) +
  geom_line(aes(y = arrivals, color = "#56B4E9"), alpha = 0.8) +
  geom_line(aes(y = departures, color = "#D55E00"), alpha = 0.8)
```
...
### EDA 2
```{r}
# Select data that relates to spending
totalSpend <- filter(covid, series_name == "Spend (actual)")

# Convert values from char to correct format
totalSpend$parameter <- as.Date(totalSpend$parameter, format="%Y-%m-%d")
totalSpend$value <- as.numeric(totalSpend$value)

# See what we gots...
plot(totalSpend$parameter, totalSpend$value)
```
