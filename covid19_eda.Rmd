...
##### Looking at indicator time periods
```{r}
# get all unique indicators
unique.indicators <- unique(covid$indicator_name)
length(unique.indicators)
```
```{r}
forex <- filter(covid, indicator_name == "Foreign exchange")
forex$date <- as.Date(forex$parameter)
```
