...
```
```{r}
ggplot(indicator.info, aes(x = time.range.year, fill = frequency, color = frequency)) + 
...
totalSpend <- filter(covid, indicator_name == "Card transaction total spend")

# Convert the 'parameter' from a string to a date
totalSpend$parameter <- as.Date(totalSpend$parameter, format="%Y-%m-%d")

# See what we gots...
plot(totalSpend$parameter, totalSpend$value)

# Go get coffee to before finding what those zero values are.
```
